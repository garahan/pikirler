...
export default function LoginPage() {
  ...
  // Memoize confetti so it doesn't recalculate randomly on re-renders
  ...
  const submit = async (e?: React.FormEvent) => {
    if (e) e.preventDefault(); // Prevent page reload from form submission
    ...
    <main className="relative grid min-h-screen place-items-center overflow-hidden px-4 py-10">
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rise-in rounded-3xl border border-white/10 bg-card/70 p-6 shadow-glowSoft backdrop-blur-xl" style={{ animationDelay: '1.1s' }}>
          ...
          {/* Changed div to form to handle Enter key presses properly */}
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="relative">
              <button 
                type="submit" 
                disabled={busy || celebrate || !username.trim() || !password}
              ...
              {celebrate && (
                <span className="pointer-events-none absolute left-1/2 top-1/2">
                  {confettiSparks.map((c) => (
                    <span 
                      key={c.id} 
                      className="confetti" 
                      style={{ background: c.color, '--cx': c.cx, '--cy': c.cy, '--cr': c.cr } as React.CSSProperties} 
                    />
                  ))}
                </span>
              )}
